...
func setupFlags() {
	...
	restoreCreateCmd.Flags().StringVar(&db, "db", "", "DB specification in the format dbType@dbUri:dbPort/dbName")
	restoreCreateCmd.Flags().StringVar(&dbUser, "db-user", "", "DB user")
	restoreCreateCmd.Flags().StringVar(&dbPass, "db-pass", "", "DB password")
	restoreCreateCmd.Flags().BoolVar(&dbUserStdin, "db-user-stdin", false, "Prompt for DB user from stdin")
	restoreCreateCmd.Flags().BoolVar(&dbPassStdin, "db-pass-stdin", false, "Prompt for DB password from stdin")
	restoreCreateCmd.Flags().StringVar(&s3, "s3", "", "S3 specification in the format endpoint:port/bucket")
	restoreCreateCmd.Flags().StringVar(&s3AccessKey, "s3-access-key", "", "S3 access key")
	restoreCreateCmd.Flags().StringVar(&s3SecretKey, "s3-secret-key", "", "S3 secret key")
	restoreCreateCmd.Flags().BoolVar(&s3AccessKeyStdin, "s3-access-key-stdin", false, "Prompt for S3 access key from stdin")
	restoreCreateCmd.Flags().BoolVar(&s3SecretKeyStdin, "s3-secret-key-stdin", false, "Prompt for S3 secret key from stdin")
	restoreCreateCmd.Flags().StringVar(&backupRevision, "revision", "0", "Revision to restore")
	restoreCreateCmd.Flags().StringVar(&backupRestoreName, "name", "", "Name of the BackupRestore")
	restoreCreateCmd.MarkFlagRequired("name")
	restoreCreateCmd.MarkFlagRequired("db")
	restoreCreateCmd.MarkFlagRequired("s3")
}
